...
import { communities, members } from '@/drizzle/schema';
...
export async function createGroup(formData: FormData) {
  ...
  try {
    ...
    if (Number(currentGroups[0].count) >= maxGroups) {
      return {
        error: `You've reached your plan limit of ${maxGroups} groups. Please upgrade your plan.`,
      };
    }
...
export async function inviteMember(formData: FormData) {
  ...
  try {
    ...
    if (Number(currentMembers[0].count) >= maxMembers) {
      return {
        error: `Group has reached the maximum of ${maxMembers} members for your plan`,
      };
    }
