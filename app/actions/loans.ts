...
export async function requestLoan(formData: FormData) {
  ...
  try {
    ...
    if (group.max_loan_amount && Number(amount) > Number(group.max_loan_amount)) {
      return { error: `Loan amount exceeds the maximum of $${Number(group.max_loan_amount).toFixed(2)}` }
    }
    ...
    if (existingLoans.length > 0) {
      return { error: "You already have an active or pending loan in this group" }
    }
    ...
    return { success: true, message: "Loan request submitted successfully. Awaiting admin approval." }
  } catch (error) {
...
export async function recordRepayment(formData: FormData) {
  ...
  try {
    ...
    if (Number(amount) > remainingBalance) {
      return { error: `Payment amount exceeds remaining balance of $${remainingBalance.toFixed(2)}` }
    }
    ...
    revalidatePath("/dashboard/loans")
    return { success: true, message: "Repayment recorded successfully. Awaiting admin approval." }
  } catch (error) {
